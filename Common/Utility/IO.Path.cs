	...
	, System.ComponentModel.INotifyPropertyChanged
	{
		...
		public string Full {
			...
			set {
				...
				if( index_name > index_ext ) index_ext = -1;

				if( index_ext  < 0 ) index_ext  = path.Length;
				if( index_name < 0 ) index_name = 0;
				else ++index_name;

				var ext  = (index_ext  >= path.Length) ? "" : path.Substring(index_ext);
		...
		/// </summary>
		public cmk.IO.Path RelativeTo( string PATH )
		{
		...
		public bool Exists => System.IO.File.Exists(Full);

		//...........................................................

		public bool EnsureExists()
		{
			if( !Exists && !Full.IsNullOrEmpty() ) {
				try { System.IO.File.Create(Full); }
		...
		public static int Compare( Path LHS, Path RHS )
		{
			...
			c = string.Compare(LHS?.Name, RHS?.Name, StringComparison.OrdinalIgnoreCase);
			if( c != 0 ) return c;

			return string.Compare(LHS?.Extension, RHS?.Extension, StringComparison.OrdinalIgnoreCase);
		}
