...
namespace cmk.NMS
{
	...
	public partial class MBINC
	{
		...
		public string MbinToEbin( string PATH, object MBIN )
		{
			...
			MbinToEbin("", "", MBIN.GetType(), MBIN, null, builder, "");
		...
		protected void MbinToEbin( string INDEX, string NAME, Type TYPE, object VALUE, FakeEnum ENUM, StringBuilder BUILDER, string INDENT )
		{
			...
			if( TryMbinNMSTemplateToEbin(index, head, NAME, VALUE, BUILDER, INDENT) ) return;
			...
			switch( VALUE ) {
				...
				case Array array_v:
					if( ENUM != null ) {
						...
						for( var i = array_v.GetLowerBound(0); i <= array_v.GetUpperBound(0); ++i ) {
							MbinToEbin($"[{i}:{ENUM.Values[i].Name}] ", "", TYPE, array_v.GetValue(i), null, BUILDER, indent);
						}
					}
					else {
						BUILDER.AppendLine($"{head}{NAME} {type_name}[{array_v.GetLength(0)}]");
						for( var i = array_v.GetLowerBound(0); i <= array_v.GetUpperBound(0); ++i ) {
							MbinToEbin($"[{i}] ", "", TYPE, array_v.GetValue(i), null, BUILDER, indent);
						}
					}
					break;
				...
				case IList list_v:
					...
					for( var i = 0; i < list_v.Count; ++i ) {
						MbinToEbin($"[{i}] ", "", TYPE, list_v[i], null, BUILDER, indent);
					}
					break;
		...
		/// </summary>
		protected bool TryMbinNMSTemplateToEbin( bool INDEX, string HEAD, string NAME, object VALUE, StringBuilder BUILDER, string INDENT )
		{
			...
			switch( type_name ) {
				...
				case "Vector4f": {
					dynamic vec = VALUE;
					BUILDER.AppendLine($"{head}[{vec.x}, {vec.y}, {vec.z}, {4}]");
					return true;
				}
			...
			if( type == NMSTemplateType || type.IsSubclassOf(NMSTemplateType) ) {
				...
				foreach( var field in type.GetFields() ) {
					...
					MbinToEbin("", field.Name, field.FieldType, value, fake_enum, BUILDER, indent);
				}
				return true;
			}
