...
namespace cmk.NMS
{
	...
	public partial class MBINC
	{
		...
		public MBINC( Assembly ASSEMBLY )
		{
			...
			m_NMSTemplate_DeserializeBinaryTemplate = NMSTemplateType?.GetMethod(
				"DeserializeBinaryTemplate", BindingFlags.Static | BindingFlags.Public
			);
			m_EXmlFile_WriteTemplate = EXmlFileType?.GetMethod(
				"WriteTemplate", BindingFlags.Static | BindingFlags.Public
			);
			m_EXmlFile_ReadTemplateFromString = EXmlFileType?.GetMethod(
				"ReadTemplateFromString", new [] { typeof(string) }
			);
		...
		/// </summary>
		public object RawToNMSTemplate( Stream RAW, string CLASS )
		{
			...
			try {
				...
				Log.Default.AddFailure(EX, $"{CLASS} - ");
				return null;
			}
		}
		...
		/// </summary>
		public Stream NMSTemplateToRaw( object NMSTEMPLATE )
		{
			if( !Valid || !IsNMSTemplate(NMSTEMPLATE) ) return new System.IO.MemoryStream();
			try {
		...
		/// </summary>
		public object ExmlToNMSTemplate( string EXML )
		{
		...
		/// </summary>
		public string NMSTemplateToExml( object NMSTEMPLATE )
		{
		...
		/// </summary>
		public CLASS_T CloneNMSTemplate<CLASS_T>( CLASS_T NMSTEMPLATE )
