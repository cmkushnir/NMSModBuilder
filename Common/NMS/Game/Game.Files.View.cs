...
namespace cmk.NMS.Game.Files
{
	...
	: cmk.NMS.PAK.Item.View
	{
		...
		protected static readonly NMS.PAK.File.Loader s_null_pak_file = new(null, null);
		...
		/// </summary>
		public override void SelectItem( IPathNode NODE )
		{
			...
			Dispatcher.Invoke(() => {
				switch( ARGS.Action ) {
					case NotifyCollectionChangedAction.Add: {
						...
						m_pak_list.Insert(ARGS.NewStartingIndex + 1, item);
						break;
					}
					case NotifyCollectionChangedAction.Remove: {
						var item = ARGS.OldItems[0] as NMS.PAK.File.Loader;
						if( PakComboBox.SelectedItem == item ) {
							PakComboBox.SelectedItem  = s_null_pak_file;
						}
						m_pak_list.Remove(item);
						break;
					}
					case NotifyCollectionChangedAction.Move: {
						var item = ARGS.NewItems[0] as NMS.PAK.File.Loader;
						m_pak_list.Move(ARGS.OldStartingIndex, ARGS.NewStartingIndex);
						break;
					}
					case NotifyCollectionChangedAction.Replace: {
						var item = ARGS.NewItems[0] as NMS.PAK.File.Loader;
						if( PakComboBox.SelectedItem == item ) {
							TreeSource = item?.InfoTree;
						}
						break;
					}
				}
