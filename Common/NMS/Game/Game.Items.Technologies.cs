...
namespace cmk.NMS.Game.Items.Technology
{
	...
	: cmk.NMS.Game.Items.Collection<NMS.Game.Items.Technology.Data>
	{
		...
		: base(GAME, 300, PAK_ITEM_COLLECTION)  // 3.71 - 282 
		{
		}
		...
		protected override void LoadMBIN()
		{
			...
			_ = Parallel.ForEach(mbin.Table, ITEM => {
				...
				if( dds == null ) {
					...
					var do_invert =
						dds_data.Path.Name.StartsWith("PORTALSYMBOL")
					;
					if( do_invert ) {
						data.Icon32 = dds.GetInverseBitmap(32);
						data.Icon48 = dds.GetInverseBitmap(48);
						data.Icon64 = dds.GetInverseBitmap(64);
					}
					else {
