...
namespace cmk.NMS.Game
{
	...
	public class Releases
	{
		...
		static Releases()
		{
			var path = Path.Combine(Resource.AppDirectory, "cmkNMSReleases.txt");
			List = Load(File.OpenRead(path));
			UpdateListFromGitHub();
		}

		//...........................................................

		public static void UpdateListFromGitHub()
		{
			if( GitHub.Disabled ) return;
			var local_first = List[0];
			try {
				// get current cmkNMSReleases.txt from GitHub
				Stream stream;
				...
				// insert new entries from GitHub into List
				var index = 0;
		...
		public static Release FindMbincTag( string MBINC_TAG )
		{
			...
			for( var i = 0; i < List.Count; ++i ) {
				if( string.Compare(List[i].MbincTag, MBINC_TAG, true) == 0 ) return List[i];
			}
