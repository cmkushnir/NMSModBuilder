...
namespace cmk.NMS.Game.Items
{
	...
	where DATA_T : cmk.NMS.Game.Items.Data
	{
		...
		{
			return List.Find(ID, ( ITEM, ID ) =>
				String.CompareNumeric(ITEM.Id, ID)
			);
		}
		...
		public void Load()
		{
			if( !m_built.IsSet ) try {
				...
				Game.Language.IdentifierChanged += ( LANGUAGE, OLD, NEW ) => UpdateLanguage(LANGUAGE);
			}
			finally { m_built.Set(); }
		}
		...
		protected void UpdateLanguage( Game.Language.Collection LANGUAGE )
		{
			...
			_ = Parallel.ForEach(m_list, ITEM => {
				ITEM.Name        = LANGUAGE.FindId(ITEM.NameId,        ITEM.NameId);
				ITEM.Description = LANGUAGE.FindId(ITEM.DescriptionId, ITEM.DescriptionId)
								 ?.Replace("\r\n",  "\n")
								  .Replace("\n \n", "\n")
								  .Replace("\n\n",  "\n");
			});
