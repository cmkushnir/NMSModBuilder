...
namespace cmk.NMS.PAK.MBIN
{
	...
	: avalon.Rendering.IBackgroundRenderer
	{
		...
		// will actually match a bunch of garbage 
		public static readonly Regex ColorRegex = new(
			@"(?<=\x28) *[0-9]+\.?[0-9]* *, *[0-9]+\.?[0-9]* *, *[0-9]+\.?[0-9]* *, *[0-9]+\.?[0-9]* *(?=\x29)",
		...
		public void Draw( avalon.Rendering.TextView VIEW, DrawingContext CONTEXT )
		{
			...
			foreach( Match match in matches ) {
				...
				if( parts == null || parts.Length != 4 ) continue;  // require (R, G, B, A)

				var r = 0.0; if( !double.TryParse(parts[0], out r) ) continue;
				var g = 0.0; if( !double.TryParse(parts[1], out g) ) continue;
				var b = 0.0; if( !double.TryParse(parts[2], out b) ) continue;
				//	var a = 0.0; if( !double.TryParse(parts[3], out a) ) continue;
